...
calcKMeans numOfClusters =
    let options = defaultKMeansOpts { kmeansClusters = False } in
...
plotClustering numberOfClusters fileName = do
    ...
    let kMeansResult = calcKMeans numberOfClusters
...
someFunc = do
    ...
    let numberOfPointsToRender = 35.0
    ...
    let singleThreadBenchmark = whnfIO $ mapM (return . calcKMeans) [2..(double2Int numberOfPointsToRender)]
    let concurrentBenchmark = whnfIO $ calcClusters >>= \fs -> return $ zipWith (curry (\(f, i) -> f i)) fs [2..(double2Int numberOfPointsToRender)]
...
testData= unsafePerformIO $ do
    ...
    x <- mapM (\line -> let [x, y] = T.split ((==) ' ') line in 
                return [(readDouble $ T.unpack x) :: Double, (readDouble $ T.unpack y) :: Double] ) v
